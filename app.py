...
def analyser_article(row):
    try:
        ...
        sentiment = mapping.get(sentiment_raw, "Neutral")
    except:
        sentiment = "Neutral"
...
langue = st.selectbox("Langue", ["all", "fr", "en", "es", "de", "it"])
pays = st.selectbox("Pays", ["all", "fr", "us", "gb", "de", "es"])

if st.button("🔍 Lancer la veille"):
    newsdata = fetch_newsdata_articles("DS Automobiles", nb_articles, lang=langue, country=pays)
    mediastack = fetch_mediastack_articles("DS Automobiles", nb_articles, lang=langue, country=pays)
